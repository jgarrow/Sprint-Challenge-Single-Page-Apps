...
> Answer: Client-side routing is useful because it gives you control over the URL and which components are rendered without having to wait for data to come back from a server. This lets our web "pages" to load faster. You can just check for matching URl's and render components accordingly. This also means that when navigating to other "pages", you can choose which components to render. This means that if you have, say, a consistent header and footer, you can choose to have those still rendered so they don't "refresh" or rerender when you navigate to a new "page".
